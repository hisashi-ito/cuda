...
struct cuspOptions {
	...
	bool parseArgs(int argc, char* argv[]) {
		...
		cerr << "Number of GPUs available: " << count << endl;
		...
		/*parse paramters*/
		while ((c = getopt(argc, argv, "i:f:d:r:g:\n")) != -1) {
			switch (c) {
			case 'i':
				_graphFileName = optarg;
				break;
			case 'f':
				_sparseMatFormat = atoi(optarg);
				if (_sparseMatFormat < 0)
					_sparseMatFormat = 0;
				if (_sparseMatFormat >= NUM_SPARSE_FORMATS)
					_sparseMatFormat = NUM_SPARSE_FORMATS - 1;
				break;
			case 'd':
